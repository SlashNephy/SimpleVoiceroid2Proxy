...
public class VoiceroidEngine : IDisposable
{
    ...
    {
        ConsoleLogger.Instance.Info("VOICEROID init started...");
        ...
        try
        {
            ...
            for (var i = 0; i < 1200; i++)
            {
                ...
                ConsoleLogger.Instance.Info("VOICEROID ready!");
        ...
        {
            ConsoleLogger.Instance.Error(exception, "VOICEROID init failed.");
    ...
    private static readonly Regex BracketRegex = new(@"<(?<command>.+?)>", RegexOptions.Compiled);
        ...
        text = BracketRegex.Replace(text, match =>
        {
            switch (match.Groups["command"].Value)
            {
                ...
                case "pause":
                    ...
                    ConsoleLogger.Instance.Info("********** => Paused.");
                    break;
                case "resume":
                    paused = false;

                    ConsoleLogger.Instance.Info("********** => Resumed.");
                    break;
                case "interrupt_enable":
                    interrupt = true;

                    ConsoleLogger.Instance.Info("********** => Interrupt enabled.");
                    break;
                case "interrupt_disable":
                    interrupt = false;

                    ConsoleLogger.Instance.Info("********** => Interrupt disabled.");
                    break;
                default:
                    return match.Value;
            }
    ...
    private async Task SpeakAsync(string text)
    {
        ...
        ConsoleLogger.Instance.Info($"=> {text}");
    }
